...
export default function ServioLandingV2() {
  return (
    <div className="min-h-screen bg-[#f8f6f2] text-[#171717] antialiased">
      ...
      <main className="mx-auto flex min-h-screen max-w-6xl flex-col px-6 py-16 md:px-12">
        ...
        {/* Momentum bar — honest pre-launch facts instead of a fake client marquee */}
        <section className="mt-20 grid grid-cols-1 gap-6 rounded-2xl border border-neutral-300 bg-white/60 px-8 py-8 sm:grid-cols-3">
          <div>
            <p className="instrument text-3xl text-neutral-900">2026</p>
            <p className="inter mt-1 text-sm text-neutral-500">Launching this year</p>
          </div>
          <div>
            <p className="instrument text-3xl text-neutral-900">10+</p>
            <p className="inter mt-1 text-sm text-neutral-500">Core modules, one platform</p>
          </div>
          <div>
            <p className="instrument text-3xl text-neutral-900">Cafés</p>
        ...
        {/* Feature deep-dive — alternating rows, Petpooja-style */}
        <section className="mt-24 space-y-20">
          <div className="grid items-center gap-10 md:grid-cols-2">
            <div>
              <p className="inter text-xs uppercase tracking-[0.2em] text-neutral-400">Front of house</p>
              ...
              <p className="inter mt-4 max-w-md leading-7 text-neutral-600">
                ...
                Table status updates the moment a bill is settled, so your floor
                staff always know what's open.
              </p>
            </div>
          ...
          <div className="grid items-center gap-10 md:grid-cols-2">
            ...
            <div className="order-1 md:order-2">
              <p className="inter text-xs uppercase tracking-[0.2em] text-neutral-400">Back of house</p>
              <h3 className="instrument mt-3 text-3xl text-neutral-900">Inventory that watches itself.</h3>
              <p className="inter mt-4 max-w-md leading-7 text-neutral-600">
                Stock levels update as orders go out, so you know what's running
                low before it becomes tomorrow's problem — no manual counts,
                no guesswork.
              </p>
            </div>
          </div>
        ...
        <section className="mt-24 border-t border-neutral-300 pt-14">
          ...
          <div className="mt-10 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
            {valueProps.map((v) => (
              <div key={v.title}>
                <h4 className="inter font-medium text-neutral-900">{v.title}</h4>
                <p className="inter mt-2 text-sm leading-6 text-neutral-500">{v.desc}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* Closing CTA band — dark, Toast-style */}
        <section className="mt-24 rounded-2xl bg-neutral-900 px-8 py-12 text-white md:px-14">
          <div className="grid items-center gap-8 md:grid-cols-2">
            <div>
              <h2 className="instrument text-4xl">Let's get your café running on Servio.</h2>
              <p className="inter mt-4 text-neutral-300">
            ...
            <div className="flex flex-col items-start gap-4 md:items-end">
              <Link
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inter rounded-full bg-white px-7 py-3 text-sm font-medium text-neutral-900 transition hover:bg-neutral-100"
              >
                Book a Demo
              </Link>
              <a
